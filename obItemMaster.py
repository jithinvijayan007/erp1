...
def itemsfromexcelsap():
    try:
        ...
        try:
            ...
            df = pd.read_excel("sap_main_master.xlsx",sheet_name="item master",header=0)
        except Exception as e:
            print('cannot read excel')
        lst_known_keys = ['Apx Code','Item No.','Item Description','Group Name','Serial No. Management','HSN','SAC Code','Brand','Category','PRODUCT CATEGORY','ITEM CATEGORY','GST','Purchase Item','Inventory Item','Sales Item']
        ...
        for ind,row in df.iterrows():
            try:
                ...
                if str(vchr_current_item_code).upper() == 'NAN' or str(vchr_current_item_code).upper() == 'NILL':
                    ...
                    #if spare product is service else sales
                    # if row['Group Name'].upper() == "SPARE":
                    #     int_sales = 2
                    # else:
                    int_sales = 1
                    ...
                    if ins_product:
                        ...
                        """category"""
                        vchr_category_name = str(row['Category'])
                        ...
                        if not ins_category:
                            cur.execute("INSERT INTO category(vchr_name,dat_created,int_status) values('"+vchr_category_name+"','"+str(datetime.now())+"',0) returning pk_bint_id;")
                            int_category_id = cur.fetchone()[0]
                        else:
                            int_category_id = ins_category[0][0]

                        cur.execute("INSERT INTO products(vchr_name,fk_category_id,dat_created,int_status)values('"+vchr_product_name+"','"+str(int_category_id)+"','"+str(datetime.now())+"',0) returning pk_bint_id; ")
                    ...
                    if not ins_brand:
                        cur.execute("INSERT INTO brands(vchr_name,int_status) values('"+vchr_brand_name+"',0) returning pk_bint_id;")
                    ...
                    row['GST'] = row['GST'] if row.get('GST') else 0
                    ...
                    if ins_item_category:
                        ...
                        cur.execute("INSERT INTO item_category(vchr_item_category,json_tax_master,json_specification_id,dat_created,vchr_hsn_code,vchr_sac_code,int_status) values('"+vchr_item_category+"','"+str(dct_tax)+"','"+str(lst_specs)+"','"+str(datetime.now())+"','"+vchr_hsn_code+"','"+vchr_sac_code+"',0) returning pk_bint_id;")
                        int_item_category_id = cur.fetchone()[0]

                    """item group"""
                    vchr_item_group = str(row['Group Name'])
                    ...
                    if ins_item_group:
                        ...
                        cur.execute("INSERT INTO item_group(vchr_item_group,dat_created,int_status) VALUES('"+vchr_item_group+"','"+str(datetime.now())+"',0) returning pk_bint_id;")
                    ...
                    cur.execute("SELECT pk_bint_id FROM item WHERE vchr_item_code = '"+str(vchr_current_item_code)+"';")
