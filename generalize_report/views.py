...
class GetChartDetails(APIView):
    ...
    def post(self,request):
        try:
            ...
            return Response({'status':1,'dct_details':str_data,'name':str_name,'url_name':str_url_name})
    ...
    def post(self,request):
        try:
            ...
            if ins_general[0]['vchr_url_name'] in ['direct_discount_report','indirect_discount_report']:
                str_filter = "dat_sale::DATE between '"+str(fromdate)+"' AND '"+str(todate)+"'"
            elif ins_general[0]['vchr_url_name'] in ['mobilefinancereport']:
                str_filter = "(em.dat_created_at::DATE between '"+str(fromdate)+"' AND '"+str(todate)+"') OR "+ "(ie.dat_sale::DATE between '"+str(fromdate)+"' AND '"+str(todate)+"')"
            ...
            if request.user.userdetails.fk_group.vchr_name.upper() in ['ADMIN','MARKETING','AUDITOR','AUDITING ADMIN','COUNTRY HEAD','GENERAL MANAGER SALES','ECOMMERCE','VIRTUAL','ASSISTANT MANAGER - ONLINE FINANCE','MANAGER BUSINESS OPERATIONS','PRODUCT MANAGER']:
                pass
            elif request.user.userdetails.fk_group.vchr_name.upper() in ['BRANCH MANAGER','ASSISTANT BRANCH MANAGER']:
                if ins_general[0]['vchr_url_name'] in ['direct_discount_report','indirect_discount_report']:
                    str_filter += " AND branch.pk_bint_id = "+str(request.user.userdetails.fk_branch_id)
                    ...
                    str_filter += " AND branch_id = "+str(request.user.userdetails.fk_branch_id)
            ...
            dctFilter = list(GeneralizeReport.objects.filter(vchr_url_name =request.data['reportname']).values('json_filter'))[0]['json_filter']
            session.close()
            return Response({'status':1,'data':dct_data,'data_chart':dct_data_chart,'filter':dctFilter})
            ...
            print(str(msg))
            return Response({'result':0,'reason':str(msg)})
