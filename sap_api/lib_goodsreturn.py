...
def GoodsReturn(date):
    ...
    try:
        try:
            ...
            conn = psycopg2.connect(host="localhost",database="pos_test", user="admin", password="tms@123")
        ...
        # if 1 == 1:
            ...
            if dct_data:
                ...
                for item in dct_data:
                    ...
                    grnrm_vchr_purchase_return_num = "GRTN-SAP-"+item['line'][0]['WhsCode']+"-"+str(item['DocNum'])
                    ...
                    #         print("Returned IMEI : ",lst_imei_updated)
                        ...
                        if lst_grn_avail and not lst_grn_avail[0]['imei_status']:
                            ...
                            # lst_batch_data.append(dict_batch)
                        ...
                        if bln_srn_reg and lst_grn_avail and lst_grn_avail[0]['imei_status']:
                            ...
                            # print(lst_data)
                            # print(grnrm_vchr_purchase_return_num)
                ...
                if lst_batch:
                    ...
                    for batch_data_main in lst_batch:
                        ...
                        for batch_data in batch_data_main['data']:
                            # import pdb; pdb.set_trace()
                            # print(batch_data)
                            # if batch_data['batch'] not in ['2286272','2283364','2269069']:
                            #     continue
                        ...
                        # cur1.execute("select gm.fk_branch_id,gd.* from grn_details gd join grn_master gm on gd.fk_purchase_id = gm.pk_bint_id where fk_branch_id="+str(int_branch_id)+" and vchr_batch_no='"+str(batch_data['batch'])+"'")
                        # ins_branch_data=cur1.fetchall()
                        # if ins_branch_data:
                            
                        #     for branch_data in ins_branch_data:
                        #         if int_stock_qty and branch_data['int_avail']>=int_stock_qty:
                        #         # print("update1")
                        #             import pdb; pdb.set_trace()
                        #             cur1.execute("update grn_details set int_avail="+str(branch_data['int_qty']-int_stock_qty)+" where pk_bint_id="+str(branch_data['pk_bint_id']))
                        #             int_stock_qty=0 
                        #             print("QTY : ",str(branch_data['int_qty']-int_stock_qty))
                        #             print("grn_details ID : ",branch_data['pk_bint_id'])
                        #             break
                        #         elif int_stock_qty and branch_data['int_avail']>0:
                        #             dct_temp = {}
                        #             dct_temp['grn_details'] = branch_data['pk_bint_id']
                        #             dct_temp['qty_returned'] = int_stock_qty
                        #             dct_temp['qty_in_branch'] = branch_data['int_qty']
                        #             dct_temp['batch_data'] = batch_data
                        #             lst_batch_not_updated.append(dct_temp)
                        #             # print("update2")
                        #             # import pdb; pdb.set_trace()
                        #             cur1.execute("update grn_details set int_avail=0 where pk_bint_id="+str(branch_data['pk_bint_id']))
                        #             int_stock_qty-=branch_data['int_avail']
                        #     # else:
                        #     #     break
                        
                        # # if int_stock_qty > 0:

                        # #     cur.execute("select bm.fk_branch_id,bd.* from branch_stock_details bd join branch_stock_master bm on \
                        # #         bd.fk_master_id  = bm.pk_bint_id where fk_branch_id="+str(int_branch_id)+" and (jsn_batch_no->>'batch')::jsonb ?'"+str(batch_data['batch'])+"'")
                        # #     ins_branch_data=cur.fetchall()
                        # #     if ins_branch_data[0]['int_qty']>=int_stock_qty:
                        # #         int_stock_qty=0
                        # #     else:
                                
                        # #         int_stock_qty-=ins_branch_data[0]['int_qty']
                        

                        # if int_stock_qty > 0:

                        #     cur1.execute("select bd.pk_bint_id as bd_id,bd.int_qty as bd_qty,bid.pk_bint_id as bid_id,bid.int_qty as bid_qty from branch_stock_details bd join branch_stock_master bm on bd.fk_master_id  = bm.pk_bint_id join branch_stock_imei_details bid on bid.fk_details_id=bd.pk_bint_id where fk_branch_id="+str(int_branch_id)+" and bid.int_qty>0 and (bd.jsn_batch_no->>'batch')::jsonb ?'"+str(batch_data['batch'])+"' and (bid.jsn_batch_no->>'batch')::jsonb ?'"+str(batch_data['batch'])+"' order by bid.pk_bint_id")
                        #     ins_branch_data=cur1.fetchall()
                        #     if ins_branch_data:
                                
                        #         for branch_data in ins_branch_data:

                        #         #cur1.execute("select * from branch_stock_imei_details where fk_details_id="+str(ins_branch_data[0]['pk_bint_id']))
                        #         #ins_branch2_data=cur1.fetchall()
                                

                        #             if branch_data['bid_qty']>=int_stock_qty:
                        #             # print("update3")
                        #                 # import pdb; pdb.set_trace()
                        #                 cur1.execute("update branch_stock_details set int_qty="+str(branch_data['bd_qty']-int_stock_qty)+" where pk_bint_id="+str(branch_data['bd_id']))
                        #                 cur1.execute("update branch_stock_imei_details set int_qty="+str(branch_data['bid_qty']-int_stock_qty)+" where pk_bint_id="+str(branch_data['bid_id']))
                        #                 int_stock_qty=0
                                        
                        #                 print("QTY : ",str(branch_data['bd_qty']-int_stock_qty))
                        #                 print("branch_stock_details ID : ",branch_data['bd_id'])
                        #                 print("branch_stock_imei_details ID : ",branch_data['bid_id'])
                        #                 break
                        #             else:
                        #             # print("update4")
                        #                 dct_temp = {}
                        #                 dct_temp['bid_id'] = branch_data['bid_id']
                        #                 dct_temp['bd_id'] = branch_data['bd_id']
                        #                 dct_temp['qty_returned'] = int_stock_qty
                        #                 dct_temp['qty_in_branch'] = branch_data['bd_qty']
                        #                 dct_temp['qty_in_branch_stock_imei_details'] = branch_data['bid_qty']
                        #                 dct_temp['batch_data'] = batch_data
                        #                 lst_batch_not_updated.append(dct_temp)

                        #                 # cur1.execute("update branch_stock_details set int_qty="+str(branch_data['bd_qty']-int_stock_qty)+" where pk_bint_id="+str(branch_data['bd_id']))
                        #                 # cur1.execute("update branch_stock_imei_details set int_qty= 0 where pk_bint_id="+str(branch_data['bid_id']))
                        #                 # int_stock_qty-=branch_data['bid_qty']

                cur.close()
                cur1.close()
                conn.close()
                # print("Batch not added grn_details : ",lst_batch_not_updated_in_grn_details)
                # print("Batch not added branch_stock and branch_stock_imei_details : ",lst_batch_not_updated_in_bsd_bsid)
                # import pdb; pdb.set_trace()
