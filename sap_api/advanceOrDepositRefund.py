...
def AdvanceRefundMethod(str_entry,doc_date):
    """
        ...
        if rst_id and rst_id[0]['array_agg']:
            ...
            for int_id in lst_ids:
                try:
                    ...
                    str_payment_query = "select rim.pk_bint_id as refund_id,p.vchr_doc_num as doc_num,p.dbl_amount as amount,rim.dat_created as dat_refund,br.vchr_code as branch_code,lm.int_code as loc,cust.txt_address as address,cust.vchr_name as cust_name,cust.vchr_code as cust_code,cust.int_mobile as cust_mobile,p.vchr_doc_num,p.dat_payment,p.int_payee_type,p.fk_bank_id,p.int_fop,r.vchr_receipt_num from receipt_invoice_matching rim join payment p on p.pk_bint_id=rim.fk_payment_id join receipt r on r.pk_bint_id=rim.fk_receipt_id join branch br on br.pk_bint_id=p.fk_branch_id join customer_details cust on cust.pk_bint_id=r.fk_customer_id join location_master lm on lm.pk_bint_id=br.fk_location_master_id where p.int_payee_type = 1 and p.pk_bint_id="+str(int_id)+";"
                    ...
                    if rst_payment:
                        ...
                        dct_header['DocDate']  = datetime.strftime(rst_payment['dat_refund'],'%Y-%m-%d')
                        dct_header['DocType']  = 'C'
                        dct_header['TransactionType']  = "Advance"
